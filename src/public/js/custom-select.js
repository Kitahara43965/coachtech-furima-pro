document.addEventListener("DOMContentLoaded", function () {
    ...
    const customSelectPlaceholderSelector = selectedOption.querySelector(
        ".custom-select-placeholder",
    );
    const customSelectPlaceholder = config.customSelectPlaceholder || "";
    ...
    const routePurchaseUpdateMethodItemId =
        config.routePurchaseUpdateMethodItemId || null;
    const customSelectId = config.customSelectId || null;
    const hiddenInput = document.getElementById("hidden-" + customSelectId);
    ...
    if (id) {
        ...
        if (selectedItem) {
            ...
            customSelectPlaceholderSelector.style.display = "none";
            selectedText.style.display = "inline";
        }
    } else {
        customSelectPlaceholderSelector.style.display = "inline";
        selectedText.style.display = "none";
        if (otherDisplay) otherDisplay.textContent = customSelectPlaceholder;
    }
    ...
    if (isDisabled) {
        ...
        items.forEach((item) => {
            item.addEventListener("click", function () {
                ...
                if (isFetch && routePurchaseUpdateMethodItemId) {
                    // サーバー送信
                    const bodyData = {
                        [customSelectId]: selectedId,
                    ...
                    fetch(routePurchaseUpdateMethodItemId, {
                        method: "POST",
                        headers: {

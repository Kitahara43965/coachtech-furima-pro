...
trait PurchaseTrait
{
    ...
    public function purchase($purchaseKind){
        ...
        $previousSelectedPurchaseTypedPivot = $previousUser->pendingItems()
        ...
        if($addressChangeMarker !== 0){
            //配送先変更画面に移動
            $response = $this->get(route('purchase.address.itemId',['item_id' => $randomItemId]));
            if($purchaseKind !== $this->purchaseUndefinedKind){
            ...
            //住所を登録
            $response = $this->post(route('purchase.address.update.itemId',['item_id'=>$randomItemId]), $formData);

            if($purchaseKind !== $this->purchaseUndefinedKind){
                //配送先住所を変更したときにバリデーションエラーが出ていないことを確認
                $response->assertSessionDoesntHaveErrors();
                
                $response->assertRedirect(route('purchase.itemId', ['item_id' => $randomItemId]));

                $response = $this->get(route('purchase.itemId',['item_id' => $randomItemId]));
        ...
        if($purchaseMethodChangeMarker !== 0){
            ...
            $response = $this->postJson(route('purchase.updateMethod.itemId',['item_id' => $randomItemId]), $formData);
        ...
        //$this->post(route('purchase.address.update.itemId',['item_id'=>$randomItemId])や
        //$this->postJson(route('purchase.updateMethod.itemId',['item_id' => $randomItemId])
        //で$nextSelectedPurchaseTypedPivotが生成されます。
        //下記のroute('purchase.store.itemId', ['item_id' => $randomItem->id])でも同様です。

        $nextSelectedPurchaseTypedPivot = $nextUser->pendingItems()
                        ?->wherePivot('item_id', $randomItemId)
                        ?->wherePivot('type', 'purchase')
                        ?->latest('created_at')
                        ?->first();
        ...
        if($purchaseDenialMarker === 0){
            ...
            $response = $this->followingRedirects()->post(
                route('purchase.store.itemId', ['item_id' => $randomItem->id]),
                $dataForm
            );
            ...
            if($purchaseKind !== $this->purchaseUndefinedKind){
                ...
                if($purchaseKind === $this->purchaseFinishKind){
                    //商品を購入できたか確認
                    $this->assertTrue($nextUser->pendingItems->contains($randomItem));
                }//$purchaseKind
            }
            ...
            //mypage(購入ページ)に入る
            $response = $this->get(route('mypage',['page'=>'deal']));
            // ビューに渡された items を取得
            $viewItems = $response->viewData('shownItems');

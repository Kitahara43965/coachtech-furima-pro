...
class SellController
{
    ...
    public function onManage(Request $request,$itemId){
        ...
        if($sellType === 'createSellType'){
            ...
            $imageChangeMarker = 2;
            $returnedRoute = route('item.itemId', ['item_id' => $itemId]);
        ...
        if($sellType === 'createSellType'){
            ...
            $item = DB::transaction(function () use ($itemId, $request, $requestCategoryId, $authenticatedUser,$imageName) {
            ...
            $item = Item::with('comments')->findOrFail($itemId);
    ...
    public function sellStore(SellRequest $request){
        $itemId = null;
        return $this->onManage($request,$itemId);
    }//store

    public function sellUpdateItemId(SellRequest $request,$item_id){
        $itemId = (int)$item_id;
        return $this->onManage($request,$itemId);
    }//store

    public function sellDeleteItemId(Request $request,$item_id){
        $itemId = (int)$item_id;
        return $this->onManage($request,$itemId);
    }//store
